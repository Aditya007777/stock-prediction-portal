...
class StockPredictionAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                recent_data = yf.download(ticker, period="150d")
                recent_close = recent_data['Close'].values.reshape(-1, 1)

                if len(recent_close) < 100:
                    predicted_next_day_price = None
                else:
                    recent_scaled = scaler.fit_transform(recent_close)
                    latest_100 = recent_scaled[-100:]
                    X_input = latest_100.reshape(1, 100, 1)
                    next_day_pred = model.predict(X_input)
                    predicted_next_day_price = float(scaler.inverse_transform(next_day_pred)[0][0])
            ...
            return Response({
                ...
                'rmse': rmse,
                'r2': r2,
                'predicted_next_day_price': predicted_next_day_price  # ✅ ADDED HERE
            })
